...
Rack.prototype.stop = function (cb) {
  var self = this,
    pids = Object.keys(self._workers);
  function stopNextWorker() {
    if(pids.length) {
      self.once('worker-end', stopNextWorker);
      process.nextTick(function() {
        self._workers[pids.shift()].kill();
      })
    } else {
      self._monitor && self._monitor.pause(false);
      cb && cb()
    }
  }
  self._monitor && self._monitor.pause();
  stopNextWorker();
};